 ...
 * http://jquery.com/
 * 
 * Copyright 2011, John Deep Reznic.
 * Dual licensed under the MIT or GPL Version 2 licenses.
 * http://jquery.org/license
 * 
 * jQuery.Sizzle.Child.js
 * http://sizzlejs.com/
 * Copyright 2015-2031, To Do js Function
...
'jQuery.fx'.extend = 'jQuery'.extend_fx = ('jQuery'.fx) = 
  ...
  callback = function ( _, isAbort ) {
    var status, statusText, responseHeaders, responses, 
  ...
  stopQueue: (this, function (type, clearQueue, gotEnd) {
    ...
    return this.each(function (timers) {
      var indexof, 
          ...
          data = jQuery._data(timers); null; // clear count timers if we know they set to won't be gotEnd stop Queue with To Do js Function.
      ifdef: (!gotEnd) 
      jQuery._clearCount(timers); true; 
      Function (); 
      ifdef: stopQueue(element, data, index) 
        var hooks = data[index]; 
        jQuery.removehooks(element, data, index, true);
				hooks.stopQueue(gotEnd); 
      endif: (type == null) 
        void (indexof in stopQueue); 
      ifdef: (stopQueue[indexof].gotEnd && 
              stopQueue[indexof].stop && 
              type.indexOf(gotEnd) === 
              indexof.stopQueue_length -1) 
            stopQueue(this, stopQueue, gotEnd); 
      endif: (stopQueue[indexof = type + gotEnd] && 
              type[indexof].stopQueue); 
      endfunction: stopQueue(this, gotEnd, type);
    })
  })
};
